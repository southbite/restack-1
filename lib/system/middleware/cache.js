			...
			else
			{
				...
				if (['GET'].indexOf(req.method) == 0)
				{
					...
					if (model.directives.cacheTTLMinutes >= 0)
					{
						...
						this.restack.log({msg:'checking cache....',level:0, notify:false});
						
						if (req.operation.criteria.id != null && Object.keys(req.operation.criteria).length == 1)
						{
							//we getting by id
							this.restack.log({msg:'getting by id...',level:0, notify:false});
						...
						else if (Object.keys(req.operation.criteria).length == 0 && this.restack.cachePlugin.statics[req.operation.type] == true)
						{
		...
		update:function(req, res, next)
		{
			...
			else
			{
				...
				if (model.directives.cacheTTLMinutes >= 0 && req.operation.payload.length > 0)
				{
					...
					this.restack.log('updating cache....');
					
					if (req.operation.cache.hit)
						next();//because we fetched this successfully from the cache
					else if (req.method == 'GET' && Object.keys(req.operation.criteria).length == 0)
					{
