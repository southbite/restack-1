		...
		update:function(req, res, next)
		{
			...
			else
			{
				...
				if (req.method != 'DELETE' && model.directives.cacheTTLMinutes >= 0 && req.operation.payload.length > 0)
				{
					...
					else if (req.method == 'GET' && (Object.keys(req.operation.criteria).length == 0 || (req.operation.criteria["deleted"] == false && Object.keys(req.operation.criteria).length == 1)))
					{
					...
					else if (req.method == 'POST' && req.operation.payload.length > 0)
					{
						...
						//for all multiple PUT, POST - clear the getall cache
				...
				else if (req.method == 'DELETE' && req.operation.criteria.id == null && req.operation.payload > 0)
					{
						...
						this.restack.cachePlugin.statics[req.operation.type] = false; //do this regardless because it is true that the data has changed
						this.restack.cachePlugin.clearAll({collection:req.operation.type}, function(e){
							if (!e)
								next();
							else
								this.failed(e, false, next);
