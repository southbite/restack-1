...
var data_helper = {
	internals:{
		...
		transform:function(mongoObject, options) {
			...
			else
			{
				...
				if (options && options.clone)
					transformed = this.clone(mongoObject);
		...
		transformToDb:function(schemaObject, options){
			...
			else
			{
				...
				if (options && options.clone)
					transformed = this.clone(schemaObject);
				
				//var transformed = schemaObject;//this.clone(schemaObject);
	...
	update:function(type, criteria, data, options, done) {
		...
		data = this.internals.transformToDb(data, {clone:true, excludeId:true, excludeSystemVersion:true});//mmm... cloning, so other items further downstream don't miss the _id attribute
		criteria = this.internals.transformToDb(criteria, {clone:true});//cloning, so other items further downstream don't miss the _id attribute
