...
var data_helper = {
	internals:{
		...
		transform:function(mongoObject, options) {
			...
			else
			{
				...
				//var transformed = mongoObject;//this.clone(mongoObject);
			    ...
			    if (options && options.extender)
			    for (var property in options.extender)
			    	transformed[property] = options.extender[property];
		...
		transformToDb:function(schemaObject, options){
			...
			else
			{
				...
				if (transformed.id != undefined)
					transformed._id = ObjectID(transformed.id);
				
				delete transformed.id;
				
				if (options && options.transients)
				options.transients.forEach(function(property, index, array){
	...
	initialize:function(params, done)
	{
		...
		this.internals.clone = require('clone');
	...
	update:function(type, criteria, data, options, done) {
		...
		var currentSystemVersion = parseInt(data.systemVersion);
		var currentId = data.id;
		
		//transform to mongo obj
		data = this.internals.transformToDb(data, {transients:['systemVersion','_id']});
		...
		collection.update(criteria, {$set: data, $inc: { systemVersion: 1 }}, options, function(err, updatedCount) {
		      if (!err)
		      {
		    	  data = this.internals.transform(data, {extender:{systemVersion:currentSystemVersion, id:currentId}});
		  		  criteria = this.internals.transform(criteria);
	...
	remove:function(type, criteria, options, done) {
		...
		criteria = this.internals.transformToDb(criteria, {clone:true});
