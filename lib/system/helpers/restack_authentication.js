...
var authenticationHelper = {
		...
		encodeToken:function(user, done){
			...
			{
				var _this = this;
				var expiryTime = _this.internals.moment.unix() + (_this.restack.settings.tokenttl * 60);
				_this.restack.log('tokenttl: ' + _this.restack.settings.tokenttl);
				_this.restack.log('unix time: ' + _this.internals.moment.unix());
				
				_this.restack.log('creating token, expiry time: ' + expiryTime);
				
				var token = _this.internals.jwt.encode({user: user,
														ttl:_this.restack.settings.tokenttl, 
														expires: expiryTime}, _this.restack.settings.authTokenSecret);
		...
		decodeToken:function(params, done){
			...
			try
			{
				...
				var decoded = _this.internals.jwt.decode(token, _this.restack.settings.authTokenSecret);
				
				//we allow for a minute, in case the backend code takes a while to sync
				if (decoded.expires + 60 < _this.internals.moment.unix())
