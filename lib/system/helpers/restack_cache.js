...
var cacheHelper = {
	...
	updateAll:function(params, done){
		...
		var _this = this;
		_this.restack.log('updating cache all');
		_this.restack.log(params);
		var totalCached = 0;
		
		this.internals.async.forEach(params.data, function(item, callback) { 
			
			_this.restack.log('caching item found in async');
			_this.restack.log(item);
			_this.update({collection:params.collection, key:item[params.keyName], data:item, ttl:params.ttl}, function(e, put_response){
				
				if (!e)
				{
					_this.restack.log('updated item: ' + item[params.keyName]);
					totalCached++;
					callback();
				}
				else
